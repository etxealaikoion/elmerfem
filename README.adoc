 ...
 * With ElmerGUI:
    **~~~bash
    sudo apt install libqwt-qt5-dev libnetcdf-dev libnetcdff-dev libmumps-dev
    sudo apt install libparmetis-dev
    
    I also needed to install qt5, using the installer from the web. I only installed Qt5.15.2. Then Qt5 requires the following dependencies
    sudo apt-get install libdrm-dev # I think it was allready installed
